  ...
  <main class="crawler-page">
    ...
    <el-dialog v-model="queuedTasksDialog" :title="`当前任务 · ${queuedTasks.length}`" width="min(1040px, calc(100vw - 32px))" class="queued-tasks-dialog" append-to-body>
      ...
      <el-table v-loading="queuedTasksLoading||queuedTasksReordering||!!queuedTaskPrioritizingId" :data="queuedTasks" row-key="id" max-height="56vh" class="queued-task-table" :row-class-name="queuedTaskRowClassName">
        ...
        <el-table-column label="操作" width="230" fixed="right"><template #default="{row}"><div class="queued-task-actions"><el-button-group class="queued-task-button-group"><el-button v-if="['RUNNING','WAITING'].includes(row.status)" size="small" :loading="queuedTaskCommandId===row.id&&queuedTaskCommand==='pause'" :disabled="Boolean(queuedTaskCommandId&&queuedTaskCommandId!==row.id)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="commandCurrentTask(row,'pause')">暂停</el-button><el-button v-if="row.status==='PAUSED'" size="small" type="primary" plain :loading="queuedTaskCommandId===row.id&&queuedTaskCommand==='resume'" :disabled="Boolean(queuedTaskCommandId&&queuedTaskCommandId!==row.id)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="commandCurrentTask(row,'resume')">继续</el-button><el-button v-if="row.status==='WAITING'" size="small" type="primary" plain :loading="queuedTaskPrioritizingId===row.id" :disabled="queuedTasksReordering||Boolean(queuedTaskCommandId)||Boolean(queuedTaskPrioritizingId&&queuedTaskPrioritizingId!==row.id)" @click="prioritizeQueuedTask(row)">优先</el-button><el-button size="small" :disabled="Boolean(queuedTaskCommandId)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="openQueuedTask(row)">详情</el-button></el-button-group></div></template></el-table-column>
      </el-table>
