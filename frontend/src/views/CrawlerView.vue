  ...
  <main class="crawler-page">
    ...
    <section v-else-if="activeTab === 'sites'" class="panel" role="tabpanel">
      ...
      <div v-if="sites.length" class="site-grid">
        <article v-for="site in sites" :key="site.id" class="site-card">
          <div class="site-top"><span class="site-mark">{{ site.siteName.slice(0, 1) }}</span><div><h3>{{ site.siteName }}</h3><a :href="site.homeUrl || site.baseUrl" target="_blank">{{ site.baseUrl }}</a></div><el-tag :type="site.status==='RULE_ERROR'?'danger':site.enabled?'success':'info'">{{ site.status==='RULE_ERROR'?'规则异常':site.enabled?'启用':'停用' }}</el-tag></div>
          ...
          <p class="health-line" :class="{error:site.status==='RULE_ERROR'}">规则 v{{ site.ruleVersion }} · {{ site.healthMessage || '尚未执行健康检查' }}</p>
          <footer><el-button text :disabled="!site.enabled || !site.rule.discoveryItemSelector" @click="scanSite(site)">扫描首页</el-button><el-button text @click="openRuleTest(site)">测试规则</el-button><el-button text @click="openVersions(site)">版本</el-button><el-button text @click="checkHealth(site)">健康检查</el-button><el-button text @click="openSite(site)">编辑</el-button><el-button text @click="openCrawl(site.id)">采集 URL</el-button><el-button text type="danger" @click="removeSite(site)">删除</el-button></footer>
        </article>
      </div>
    ...
    <el-dialog v-model="siteDialog" :title="editingSite ? '编辑采集网站' : '新增采集网站'" width="min(760px, 94vw)" destroy-on-close>
      <el-form label-position="top" class="site-form">
        ...
        <div class="rule-block"><p class="eyebrow">DISCOVERY SELECTORS</p><h3>首页发现规则</h3><p>自动扫描需要配置书籍项和详情链接；其余字段可按站点页面结构选填。</p></div>
        <div class="form-grid"><el-form-item label="书籍项 Selector"><el-input v-model="siteForm.rule.discoveryItemSelector" placeholder=".book-list .book" /></el-form-item><el-form-item label="详情链接 Selector"><el-input v-model="siteForm.rule.discoveryUrlSelector" placeholder="a.book-link" /></el-form-item></div>
        <div class="form-grid"><el-form-item label="书名 Selector"><el-input v-model="siteForm.rule.discoveryTitleSelector" placeholder=".title" /></el-form-item><el-form-item label="作者 Selector"><el-input v-model="siteForm.rule.discoveryAuthorSelector" placeholder=".author" /></el-form-item></div>
        <div class="form-grid"><el-form-item label="封面 Selector"><el-input v-model="siteForm.rule.discoveryCoverSelector" placeholder="img.cover::data-src" /></el-form-item><el-form-item label="分类 Selector"><el-input v-model="siteForm.rule.discoveryCategorySelector" placeholder=".category" /></el-form-item></div>
        <div class="form-grid"><el-form-item label="最新章节 Selector"><el-input v-model="siteForm.rule.discoveryLatestChapterSelector" placeholder=".latest" /></el-form-item><el-form-item label="下一页 Selector"><el-input v-model="siteForm.rule.discoveryNextPageSelector" placeholder="a.next" /></el-form-item></div>
        ...
        <el-form-item label="XPath 删除规则（每行一条）"><el-input v-model="siteForm.rule.xpathRemoveSelectors" type="textarea" placeholder="//div[contains(@class,'watermark')]" /></el-form-item>
        <el-form-item label="字符串替换 JSON"><el-input v-model="siteForm.rule.stringReplacementsJson" type="textarea" placeholder='{"请下载本站APP":""}' /></el-form-item>
        ...
        <div class="switch-row"><el-switch v-model="siteForm.rule.removeBlankLines" active-text="移除空白行"/><el-switch v-model="siteForm.rule.saveOriginalHtml" active-text="保留原始正文 HTML"/></div>
        <el-alert type="warning" :closable="false" title="仅采集你有权阅读的免费内容；系统不会绕过登录、付费、验证码或访问控制。" />
      </el-form>
      <template #footer><el-button v-if="editingSite" @click="openRuleTest(editingSite,true)">在线测试当前规则</el-button><el-button @click="siteDialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="saveSite">保存网站与规则</el-button></template>
    </el-dialog>
    ...
    <el-drawer v-model="versionsDrawer" title="规则版本历史" size="min(560px, 94vw)">
      <div class="version-list"><article v-for="version in ruleVersions" :key="version.id"><div><b>Version {{ version.version }}</b><p>{{ version.changeSummary }} · {{ formatTime(version.createdAt) }}</p></div><el-button text type="primary" :disabled="version.version===versionSite?.ruleVersion" @click="restoreVersion(version.id)">恢复为新版本</el-button></article></div>
      <el-empty v-if="!ruleVersions.length" description="保存规则后会生成版本快照" />
    </el-drawer>
...
import { crawlerApi, type CrawlerBook, type CrawlerChapter, type CrawlerDashboard, type CrawlerRuleTest, type CrawlerRuleVersion, type CrawlerSite, type CrawlerSitePayload, type CrawlerTask } from '@/utils/crawler'
...
const siteDialog=ref(false), crawlDialog=ref(false), bookDrawer=ref(false), chapterDialog=ref(false), ruleTestDialog=ref(false), versionsDrawer=ref(false), saving=ref(false), testingRule=ref(false), editingSite=ref<CrawlerSite>(), selectedBook=ref<CrawlerBook>(), selectedDiscoveries=ref<CrawlerBook[]>([]), chapters=ref<CrawlerChapter[]>([]), chapterDetail=ref<{title:string;url:string;content:string;errorMessage:string}>(), bookKeyword=ref('')
const ruleTestSite=ref<CrawlerSite>(), versionSite=ref<CrawlerSite>(), ruleTestUrl=ref(''), ruleTestUsesDraft=ref(false), ruleTestResult=ref<CrawlerRuleTest>(), ruleVersions=ref<CrawlerRuleVersion[]>([])
const crawlForm=reactive<{siteId?:number;url:string}>({url:''})
const emptySite=():CrawlerSitePayload=>({siteName:'',siteCode:'',baseUrl:'',homeUrl:'',enabled:false,autoScan:false,autoCrawl:false,autoUpdate:true,autoImportLibrary:false,scanIntervalMinutes:360,updateIntervalMinutes:30,maxDiscoveryPages:3,autoImportFormat:'EPUB',requestIntervalMillis:1500,randomDelayMillis:1000,maxConcurrency:1,timeoutMillis:15000,retryCount:2,encoding:'UTF-8',rule:{discoveryItemSelector:'',discoveryUrlSelector:'a',discoveryTitleSelector:'.title',discoveryAuthorSelector:'',discoveryCoverSelector:'',discoveryCategorySelector:'',discoveryLatestChapterSelector:'',discoveryNextPageSelector:'',titleSelector:'',authorSelector:'',descriptionSelector:'',chapterListUrlSelector:'',chapterItemSelector:'',chapterTitleSelector:':scope',chapterUrlSelector:'a',contentTitleSelector:'h1',contentSelector:'',removeSelectors:'',xpathRemoveSelectors:'',stringReplacementsJson:'',regexReplacementsJson:'',removeBlankLines:true,saveOriginalHtml:false,minChapterLength:100}})
...
async function saveSite(){if(!siteForm.siteName||!siteForm.siteCode||!siteForm.baseUrl||!siteForm.rule.titleSelector||!siteForm.rule.chapterItemSelector||!siteForm.rule.contentSelector)return message.warning('请填写网站、书名、章节列表和正文的必要规则');if(siteForm.autoScan&&(!siteForm.rule.discoveryItemSelector||!siteForm.rule.discoveryUrlSelector))return message.warning('开启自动扫描时，请配置书籍项与详情链接 Selector');saving.value=true;try{editingSite.value?await crawlerApi.updateSite(editingSite.value.id,siteForm):await crawlerApi.createSite(siteForm);message.success('网站与解析规则已保存');siteDialog.value=false;await refresh()}finally{saving.value=false}}
...
function openRuleTest(site:CrawlerSite,useDraft=false){ruleTestSite.value=site;ruleTestUsesDraft.value=useDraft;ruleTestResult.value=undefined;ruleTestUrl.value='';ruleTestDialog.value=true}
async function runRuleTest(){if(!ruleTestSite.value||!ruleTestUrl.value)return message.warning('请填写同站点的书籍详情 URL');testingRule.value=true;try{ruleTestResult.value=await crawlerApi.testRule(ruleTestSite.value.id,ruleTestUrl.value,ruleTestUsesDraft.value?siteForm.rule:undefined)}finally{testingRule.value=false}}
...
async function openVersions(site:CrawlerSite){versionSite.value=site;ruleVersions.value=await crawlerApi.ruleVersions(site.id);versionsDrawer.value=true}
async function restoreVersion(versionId:number){if(!versionSite.value||!await confirm('恢复后会以历史配置创建一个新的规则版本，是否继续？'))return;const siteId=versionSite.value.id;await crawlerApi.restoreRuleVersion(siteId,versionId);message.success('历史规则已恢复并生成新版本');await refresh();versionSite.value=sites.value.find(s=>s.id===siteId);ruleVersions.value=await crawlerApi.ruleVersions(siteId)}
...
.site-card footer{flex-wrap:wrap}.health-line{margin-top:12px;color:var(--text-tertiary);font-size:11px}.health-line.error{color:var(--danger)}.test-result{padding:18px;border:1px solid var(--success-alpha-15);border-radius:16px;background:var(--surface-elevated)}.test-result.failed{border-color:var(--danger)}.test-facts{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.test-facts span{display:grid;gap:4px}.test-facts small{color:var(--text-tertiary)}.test-result>p{margin:18px 0 8px}.test-result pre{max-height:260px;overflow:auto;padding:14px;border-radius:12px;background:var(--bg-page);font:14px/1.7 'Songti SC',serif;white-space:pre-wrap}.version-list{display:grid;gap:10px}.version-list article{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px;border:1px solid var(--border-color-light);border-radius:14px;background:var(--surface-elevated)}.version-list p{margin-top:4px;color:var(--text-secondary);font-size:12px}
@media(max-width:900px){.metric-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.site-grid{grid-template-columns:minmax(0,1fr)}}@media(max-width:640px){.hero{align-items:flex-start;flex-direction:column;gap:22px;padding:24px}.hero h1{font-size:34px}.panel{padding:16px}.metric-grid,.form-grid{grid-template-columns:minmax(0,1fr)}.segmented-wrap{justify-content:start}.section-heading{align-items:flex-start;flex-direction:column;gap:12px}.search{width:100%}}@media(prefers-reduced-motion:reduce){.segment-indicator{transition:none}}
</style>
