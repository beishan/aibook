<template>
  <main class="crawler-page">
    ...
    <section v-else-if="activeTab === 'books'" class="panel" role="tabpanel">
      <div class="section-heading"><div><p class="eyebrow">STRUCTURED BOOKS</p><h2>采集书籍</h2></div><div class="book-search"><el-input v-model="bookKeyword" clearable placeholder="搜索书名、作者、网站或编码" :prefix-icon="Search" class="search" @keyup.enter="applyBookFilters" @clear="applyBookFilters" /><el-button type="primary" :icon="Search" @click="applyBookFilters">查询</el-button></div></div>
...
async function refresh(){const [dashboardData,siteData]=await Promise.all([crawlerApi.dashboard(),crawlerApi.sites(),loadBooks(),loadTasks(),loadFailedTasks(),loadDiscoveredBooks()]);dashboard.value=dashboardData;sites.value=siteData}
