<template>
  <main class="crawler-page">
    ...
    <section v-else-if="activeTab === 'books'" class="panel" role="tabpanel">
      ...
      <transition name="task-batch-rise">
        <div v-if="selectedBooks.length" class="task-batch-bar book-batch-bar" role="toolbar" aria-label="采集书籍批量修改">
          <div class="task-batch-summary"><span>{{ selectedBooks.length }}</span><div><strong>已选择书籍</strong><small>统一修改所选书籍的采集状态</small></div></div>
          <div class="book-batch-actions"><el-select v-model="batchBookStatus" aria-label="批量设置采集状态"><el-option v-for="item in manualStatusOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select><el-button type="primary" :loading="batchBookStatusSaving" @click="saveBatchBookStatus">统一更改状态</el-button></div>
        </div>
      </transition>
        ...
        <div class="discovery-card-grid">
          <article v-for="book in books" :key="book.id" class="discovery-card crawler-book-card" :class="{selected:isBookSelected(book)}" @click="openBook(book)">
            <div class="discovery-card-action-overlay crawler-book-action-overlay" role="group" :aria-label="`${book.bookName}快捷操作`" @click.stop>
              <el-button size="small" type="primary" @click="continueCrawl(book)">继续</el-button>
            ...
            <div class="discovery-card-body crawler-book-card-body"><div class="discovery-card-title"><div><button type="button" class="crawler-book-title-button" :title="book.bookName" @click.stop="openBook(book)">{{ book.bookName }}</button><p>{{ book.author || '未知作者' }}</p></div><el-tag v-if="book.category" size="small" effect="plain">{{ book.category }}</el-tag></div><dl><div><dt>来源网站</dt><dd>{{ book.siteName }}</dd></div></dl><div class="crawler-book-progress"><div><span>采集进度</span><strong>{{ book.crawledChapterCount }} / {{ book.chapterCount }} 章</strong></div><el-progress :class="{'crawler-running-progress':isBookRunning(book)}" :percentage="progress(book)" :stroke-width="7" /></div></div>
          </article>
        </div>
      </div>
    ...
    <el-dialog v-model="queuedTasksDialog" :title="`当前排队任务 · ${queuedTasks.length}`" width="min(760px, 94vw)" append-to-body>
      <div v-if="queuedTasks.length" class="queued-order-note"><span class="queue-drag-mark">⠿</span><div><strong>拖动调整执行顺序</strong><small>同一优先级内越靠上越先执行；需要跨优先级置顶时可点击“立刻优先”。</small></div></div>
      <el-table v-loading="queuedTasksLoading||queuedTasksReordering||!!queuedTaskPrioritizingId" :data="queuedTasks" row-key="id" max-height="56vh" class="queued-task-table" :row-class-name="queuedTaskRowClassName">
        <el-table-column label="排序" width="62" align="center"><template #default="{row}"><button type="button" class="queue-drag-handle" :class="{dragging:queuedTaskDraggingId===row.id}" :draggable="!queuedTasksReordering&&!queuedTaskPrioritizingId" :aria-label="`拖动排序：${row.bookName||row.discoveryPageName||taskTypeLabel(row.type)}`" title="拖动调整顺序；也可使用上下方向键" @dragstart="startQueuedTaskDrag(row,$event)" @dragenter.prevent="moveDraggedQueuedTask(row)" @dragover.prevent @dragend="finishQueuedTaskDrag" @keydown.up.prevent="moveQueuedTaskByKeyboard(row,-1)" @keydown.down.prevent="moveQueuedTaskByKeyboard(row,1)">⠿</button></template></el-table-column>
        <el-table-column type="index" label="#" width="54" />
        <el-table-column label="任务" min-width="250"><template #default="{row}"><button type="button" class="queue-task-link" @dragenter.prevent="moveDraggedQueuedTask(row)" @click="openQueuedTask(row)"><strong>{{ row.bookName || row.discoveryPageName || taskTypeLabel(row.type) }}</strong><small>{{ row.siteName }} · {{ taskTypeLabel(row.type) }}</small></button></template></el-table-column>
        ...
        <el-table-column label="操作" width="178"><template #default="{row}"><div class="queued-task-actions"><el-button text type="primary" :loading="queuedTaskPrioritizingId===row.id" :disabled="queuedTasksReordering||Boolean(queuedTaskPrioritizingId&&queuedTaskPrioritizingId!==row.id)" @click="prioritizeQueuedTask(row)">立刻优先</el-button><el-button text @click="openQueuedTask(row)">详情</el-button></div></template></el-table-column>
      </el-table>
...
const queueSettingsDialog=ref(false), savingQueueSettings=ref(false), taskQueueSettings=ref<CrawlerTaskQueueSettings>(), queueLimit=ref(4), queuedTasksDialog=ref(false), queuedTasksLoading=ref(false), queuedTasksReordering=ref(false), queuedTaskPrioritizingId=ref<string>(), queuedTaskDraggingId=ref<string>(), queuedTaskDragStartOrder=ref<string[]>([]), queuedTasks=ref<CrawlerTask[]>([])
...
async function loadQueuedTasks(options:LoadOptions={}){if(options.silent&&(queuedTaskDraggingId.value||queuedTasksReordering.value||queuedTaskPrioritizingId.value))return;if(!options.silent)queuedTasksLoading.value=true;try{queuedTasks.value=await crawlerApi.queuedTasks()}finally{if(!options.silent)queuedTasksLoading.value=false}}
async function openQueuedTask(task:CrawlerTask){queuedTasksDialog.value=false;await openTask(task)}
async function prioritizeQueuedTask(task:CrawlerTask){if(queuedTasksReordering.value||queuedTaskPrioritizingId.value)return;queuedTaskPrioritizingId.value=task.id;try{queuedTasks.value=await crawlerApi.prioritizeQueuedTask(task.id);message.success(`“${task.bookName||task.discoveryPageName||taskTypeLabel(task.type)}”已移到等待队列首位`);await Promise.all([loadTasks({silent:true}),loadTaskQueueSettings()])}catch(error:any){message.error(error.response?.data?.message||'任务优先调整失败');await loadQueuedTasks()}finally{queuedTaskPrioritizingId.value=undefined}}
function startQueuedTaskDrag(task:CrawlerTask,event:DragEvent){if(queuedTasksReordering.value){event.preventDefault();return}queuedTaskDraggingId.value=task.id;queuedTaskDragStartOrder.value=queuedTasks.value.map(item=>item.id);if(event.dataTransfer){event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',task.id)}}
function moveDraggedQueuedTask(target:CrawlerTask){const sourceIndex=queuedTasks.value.findIndex(item=>item.id===queuedTaskDraggingId.value),targetIndex=queuedTasks.value.findIndex(item=>item.id===target.id);if(sourceIndex<0||targetIndex<0||sourceIndex===targetIndex||queuedTasks.value[sourceIndex].priority!==target.priority)return;const next=[...queuedTasks.value],[source]=next.splice(sourceIndex,1);next.splice(targetIndex,0,source);queuedTasks.value=next}
async function persistQueuedTaskOrder(){queuedTasksReordering.value=true;try{queuedTasks.value=await crawlerApi.reorderQueuedTasks(queuedTasks.value.map(task=>task.id));message.success('等待队列顺序已更新')}catch(error:any){message.error(error.response?.data?.message||'队列排序保存失败');await loadQueuedTasks()}finally{queuedTasksReordering.value=false}}
async function finishQueuedTaskDrag(){const changed=queuedTaskDragStartOrder.value.join(',')!==queuedTasks.value.map(task=>task.id).join(',');queuedTaskDraggingId.value=undefined;queuedTaskDragStartOrder.value=[];if(changed)await persistQueuedTaskOrder()}
async function moveQueuedTaskByKeyboard(task:CrawlerTask,direction:-1|1){if(queuedTasksReordering.value)return;const index=queuedTasks.value.findIndex(item=>item.id===task.id),target=index+direction;if(index<0||target<0||target>=queuedTasks.value.length)return;if(queuedTasks.value[target].priority!==task.priority)return message.warning('跨优先级排序请先修改任务优先级');const next=[...queuedTasks.value];[next[index],next[target]]=[next[target],next[index]];queuedTasks.value=next;await persistQueuedTaskOrder()}
...
.book-batch-actions{display:flex;align-items:center;gap:8px}.book-batch-actions :deep(.el-select){width:170px}.book-batch-actions .el-button{margin-left:0}@media(max-width:600px){.book-batch-actions{display:grid;grid-template-columns:minmax(0,1fr)}.book-batch-actions :deep(.el-select),.book-batch-actions .el-button{width:100%}}
