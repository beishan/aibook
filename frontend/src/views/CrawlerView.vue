  ...
  <main class="crawler-page">
    ...
    <section v-else-if="activeTab === 'sites'" class="panel" role="tabpanel">
      ...
      <div v-if="sites.length" class="site-grid">
        <article v-for="site in sites" :key="site.id" class="site-card">
          ...
          <div class="automation"><span class="on">仅手动触发任务</span></div>
    ...
    <el-dialog v-model="siteDialog" :title="editingSite ? '编辑采集网站' : '新增采集网站'" width="min(860px, 96vw)" top="5vh" class="site-editor-dialog" append-to-body destroy-on-close>
      <el-form ref="siteFormRef" :model="siteForm" :rules="siteFormRules" label-position="top" class="site-form" scroll-to-error>
        ...
        <div v-show="activeSiteTab==='request'" id="site-editor-panel-request" class="site-tab-panel" role="tabpanel" aria-labelledby="site-editor-tab-request">
          <section class="site-form-section"><header><span>02</span><div><h3>请求策略</h3><p>控制访问频率、并发和失败恢复</p></div></header><div class="form-grid"><el-form-item label="请求间隔（ms）"><el-input-number v-model="siteForm.requestIntervalMillis" :min="100" :step="100" /></el-form-item><el-form-item label="随机延迟（ms）"><el-input-number v-model="siteForm.randomDelayMillis" :min="0" :step="100" /></el-form-item></div><div class="form-grid"><el-form-item label="最大并发"><el-input-number v-model="siteForm.maxConcurrency" :min="1" :max="8" /></el-form-item><el-form-item label="超时（ms）"><el-input-number v-model="siteForm.timeoutMillis" :min="1000" :step="1000" /></el-form-item></div><div class="form-grid"><el-form-item label="单次请求失败重试"><el-input-number v-model="siteForm.retryCount" :min="0" :max="8" /></el-form-item><el-form-item label="任务连续请求失败上限"><el-input-number v-model="siteForm.maxConsecutiveFailures" :min="1" :max="100" /><small class="field-hint">连续失败达到上限后停止整项任务；请求成功一次即重新计数。</small></el-form-item></div></section>
        ...
        <div v-show="activeSiteTab==='automation'" id="site-editor-panel-automation" class="site-tab-panel" role="tabpanel" aria-labelledby="site-editor-tab-automation">
          <section class="site-form-section"><header><span>06</span><div><h3>运行设置</h3><p>网站启用状态与人工扫描范围</p></div></header><div class="form-grid"><el-form-item label="单次最多扫描页数"><el-input-number v-model="siteForm.maxDiscoveryPages" :min="1" :max="50" /></el-form-item><el-form-item label="网站状态"><el-switch v-model="siteForm.enabled" active-text="启用网站" inactive-text="停用网站" /></el-form-item></div><el-alert type="info" :closable="false" title="扫描首页、采集书籍、检查更新、生成文件和入库均只会由人工操作触发。" /></section>
    ...
    <el-dialog v-model="ruleEditorDialog" :title="editingRule ? `编辑规则 v${editingRule.version}` : '新建规则版本'" width="min(800px, 96vw)" append-to-body destroy-on-close>
      <el-form label-position="top" class="site-form">
        ...
        <div class="rule-block"><p class="eyebrow">DISCOVERY SELECTORS</p><h3>书籍列表规则</h3><p>人工扫描首页时需要配置书籍项和详情链接；其余字段可选填。</p></div>
...
const emptySite=():CrawlerSitePayload=>({siteName:'',siteCode:'',baseUrl:'',homeUrl:'',enabled:false,autoScan:false,autoCrawl:false,autoUpdate:false,autoImportLibrary:false,scanIntervalMinutes:360,updateIntervalMinutes:30,maxDiscoveryPages:3,autoImportFormat:'EPUB',requestIntervalMillis:1500,randomDelayMillis:1000,maxConcurrency:1,timeoutMillis:15000,retryCount:2,maxConsecutiveFailures:5,encoding:'UTF-8',userAgent:'',cookie:'',headersJson:'',proxies:[],contentFailureMarkers:[]})
...
const siteEditorTabs:{key:SiteEditorTab;label:string;description:string}[]=[{key:'basic',label:'基本信息',description:'身份与入口'},{key:'request',label:'请求访问',description:'频率与请求头'},{key:'validation',label:'内容校验',description:'失败特征'},{key:'proxy',label:'代理配置',description:'站点代理'},{key:'automation',label:'运行设置',description:'手动任务'}]
...
function siteTabForField(field:string):SiteEditorTab{
  ...
  if(['requestIntervalMillis','randomDelayMillis','maxConcurrency','timeoutMillis','retryCount','maxConsecutiveFailures','userAgent','cookie','headersJson'].includes(field))return'request'
