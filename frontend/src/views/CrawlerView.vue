<template>
  <main class="crawler-page">
    ...
    <section v-if="activeTab === 'overview'" class="panel" role="tabpanel">
      ...
      <TaskTable :tasks="dashboard?.recentTasks || []" @command="runTaskCommand" @edit="openTaskEditor" @delete="removeTask" />
    </section>
    ...
    <section v-else-if="activeTab === 'discovered'" class="panel" role="tabpanel">
      ...
      <div class="discovery-toolbar">
        <el-input v-model="discoveryKeyword" clearable :prefix-icon="Search" placeholder="搜索书名、作者、网站、编码或最新章节" @keyup.enter="applyDiscoveryFilters" />
      ...
      <el-table v-loading="discoveryLoading" :data="discoveredBooks" class="data-table" @selection-change="selectedDiscoveries=$event">
    ...
    <section v-else class="panel" role="tabpanel">
      ...
      <TaskTable :tasks="activeTab === 'failed' ? failedTasks : tasks" @command="runTaskCommand" @edit="openTaskEditor" @delete="removeTask" />
    </section>
    ...
    <el-dialog v-model="siteDialog" :title="editingSite ? '编辑采集网站' : '新增采集网站'" width="min(860px, 96vw)" top="5vh" class="site-editor-dialog" append-to-body destroy-on-close>
      <el-form ref="siteFormRef" :model="siteForm" :rules="siteFormRules" label-position="top" class="site-form" scroll-to-error>
        ...
        <div v-show="activeSiteTab==='request'" id="site-editor-panel-request" class="site-tab-panel" role="tabpanel" aria-labelledby="site-editor-tab-request">
          <section class="site-form-section"><header><span>02</span><div><h3>请求策略</h3><p>控制访问频率、并发和失败恢复</p></div></header><div class="form-grid"><el-form-item label="请求间隔（ms）"><el-input-number v-model="siteForm.requestIntervalMillis" :min="100" :step="100" /></el-form-item><el-form-item label="随机延迟（ms）"><el-input-number v-model="siteForm.randomDelayMillis" :min="0" :step="100" /></el-form-item></div><div class="form-grid"><el-form-item label="最大并发"><el-input-number v-model="siteForm.maxConcurrency" :min="1" :max="8" /></el-form-item><el-form-item label="超时（ms）"><el-input-number v-model="siteForm.timeoutMillis" :min="1000" :step="1000" /></el-form-item></div><el-form-item label="失败重试"><el-input-number v-model="siteForm.retryCount" :min="0" :max="8" /></el-form-item></section>
          <section class="site-form-section"><header><span>03</span><div><h3>身份与请求头</h3><p>按目标网站要求附加请求身份</p></div></header><el-form-item label="User-Agent"><el-input v-model="siteForm.userAgent" placeholder="留空时使用合规的 AiBookCrawler 标识" /></el-form-item><el-form-item label="Cookie"><el-input v-model="siteForm.cookie" type="password" show-password autocomplete="off" /></el-form-item><el-form-item label="自定义 Header JSON"><el-input v-model="siteForm.headersJson" type="textarea" placeholder='{"Referer":"https://example.com/"}' /></el-form-item></section>
        </div>
        ...
        <div v-show="activeSiteTab==='automation'" id="site-editor-panel-automation" class="site-tab-panel" role="tabpanel" aria-labelledby="site-editor-tab-automation">
          <section class="site-form-section"><header><span>06</span><div><h3>自动化</h3><p>扫描、更新、采集与入库策略</p></div></header><div class="form-grid"><el-form-item label="首页扫描间隔（分钟）"><el-input-number v-model="siteForm.scanIntervalMinutes" :min="1" /></el-form-item><el-form-item label="更新检查间隔（分钟）"><el-input-number v-model="siteForm.updateIntervalMinutes" :min="1" /></el-form-item></div><div class="form-grid"><el-form-item label="单次最多扫描页数"><el-input-number v-model="siteForm.maxDiscoveryPages" :min="1" :max="50" /></el-form-item><el-form-item label="自动入库格式"><el-select v-model="siteForm.autoImportFormat"><el-option label="EPUB" value="EPUB"/><el-option label="TXT" value="TXT"/><el-option label="TXT + EPUB" value="BOTH"/></el-select></el-form-item></div><div class="switch-row"><el-switch v-model="siteForm.enabled" active-text="启用网站"/><el-switch v-model="siteForm.autoScan" active-text="自动扫描首页"/><el-switch v-model="siteForm.autoUpdate" active-text="自动检查更新"/><el-switch v-model="siteForm.autoCrawl" active-text="自动采集"/><el-switch v-model="siteForm.autoImportLibrary" active-text="自动入库"/></div></section>
    ...
    <el-dialog v-model="statusDialog" title="人工修改采集状态" width="min(520px, 94vw)" append-to-body>
      <div v-if="statusBook" class="status-dialog-content">
        ...
        <el-form label-position="top"><el-form-item label="修改为"><el-select v-model="manualStatus" @change="handleManualStatusChange"><el-option v-for="item in manualStatusOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item><el-form-item label="自动追更"><el-switch v-model="manualAutoUpdateEnabled" active-text="继续定时检查更新" inactive-text="停止自动创建更新任务" /></el-form-item></el-form>
        <el-alert type="warning" :closable="false" title="标记为“已完成”时默认停止自动追更；只调整整本书状态，不会补齐缺失章节或改变章节统计。" />
      </div>
    ...
    <el-dialog v-model="taskEditDialog" title="修改采集任务" width="min(520px, 94vw)" append-to-body>
      <div v-if="editingTask" class="task-edit-content">
        <div class="task-edit-summary"><span>{{ taskTypeLabel(editingTask.type) }}</span><div><strong>{{ editingTask.bookName || editingTask.siteName }}</strong><p>{{ editingTask.siteName }} · {{ statusLabel(editingTask.status) }}</p></div></div>
    ...
    <el-drawer v-model="bookDrawer" size="min(760px, 96vw)" :title="selectedBook?.bookName || '采集书籍详情'" class="book-detail-drawer">
      <div v-if="selectedBook" class="book-drawer-content">
        ...
        <div class="drawer-actions"><el-button @click="continueCrawl(selectedBook)">继续采集</el-button><el-button :disabled="!selectedBook.failedChapterCount" @click="retryFailures(selectedBook)">重试失败</el-button><el-button @click="openStatusEditor(selectedBook)">修改状态</el-button><el-button type="primary" @click="generate(selectedBook)">生成 TXT + EPUB</el-button><el-button type="primary" plain :disabled="selectedBook.importStatus==='IMPORTED'" @click="importBook(selectedBook)">入库</el-button></div>
        <div class="book-detail-tabs" role="tablist" aria-label="书籍采集详情" @keydown="handleBookDetailTabKey"><span class="book-detail-tab-indicator" :style="{transform:`translateX(${bookDetailTabIndex*100}%)`}"/><button v-for="item in bookDetailTabs" :key="item.value" type="button" role="tab" :aria-selected="bookDetailTab===item.value" :tabindex="bookDetailTab===item.value?0:-1" :class="{active:bookDetailTab===item.value}" @click="bookDetailTab=item.value"><span>{{ item.label }}</span><b>{{ item.value==='chapters'?chapters.length:crawlerLogs.length }}</b></button></div>
        <section v-show="bookDetailTab==='chapters'" class="drawer-detail-panel chapter-progress-panel" role="tabpanel">
          <div class="chapter-list-heading"><div><p class="eyebrow">CHAPTER INDEX</p><h3>章节进度</h3></div><div class="chapter-sort" role="tablist" aria-label="章节排序方式" @keydown="handleChapterSortKey"><span class="chapter-sort-indicator" :style="{transform:`translateX(${chapterSortIndex*100}%)`}"/><button v-for="item in chapterSortOptions" :key="item.value" type="button" role="tab" :aria-selected="chapterSort===item.value" :tabindex="chapterSort===item.value?0:-1" :class="{active:chapterSort===item.value}" @click="chapterSort=item.value">{{ item.label }}</button></div></div>
          <el-table :data="sortedChapters" height="100%" class="chapters-table" @row-click="openChapter"><el-table-column prop="chapterIndex" label="#" width="65"/><el-table-column prop="chapterName" label="章节" min-width="220"/><el-table-column prop="wordCount" label="字数" width="90"/><el-table-column label="状态" width="120"><template #default="{row}"><el-tag :type="statusType(row.crawlStatus)">{{ statusLabel(row.crawlStatus) }}</el-tag></template></el-table-column></el-table>
        </section>
        <section v-show="bookDetailTab==='logs'" class="drawer-detail-panel realtime-log-panel" role="tabpanel">
          <div class="realtime-log-heading"><div><p class="eyebrow">LIVE CRAWLER FEED</p><h3>实时日志</h3></div><span class="live-state"><i/>每 3 秒同步</span></div>
...
const TaskTable = defineComponent({ props:{ tasks:{type:Array as ()=>CrawlerTask[],required:true}}, emits:['command','edit','delete'], setup(props,{emit}) { return () => h(ElTable,{data:props.tasks,class:'data-table'},()=>[
  ...
  h(ElTableColumn,{label:'操作',width:280},{default:({row}:{row:CrawlerTask})=>[['WAITING','PAUSED','FAILED'].includes(row.status)?h(ElButton,{text:true,onClick:()=>emit('edit',row)},()=> '修改'):null,row.status==='RUNNING'?h(ElButton,{text:true,onClick:()=>emit('command',row,'pause')},()=> '暂停'):null,row.status==='PAUSED'?h(ElButton,{text:true,onClick:()=>emit('command',row,'resume')},()=> '继续'):null,['RUNNING','WAITING','PAUSED'].includes(row.status)?h(ElButton,{text:true,type:'danger',onClick:()=>emit('command',row,'cancel')},()=> '取消'):null,row.status!=='RUNNING'?h(ElButton,{text:true,type:'danger',onClick:()=>emit('delete',row)},()=> '删除'):null]})
]) }})
...
const emptySite=():CrawlerSitePayload=>({siteName:'',siteCode:'',baseUrl:'',homeUrl:'',enabled:false,autoScan:false,autoCrawl:false,autoUpdate:true,autoImportLibrary:false,scanIntervalMinutes:360,updateIntervalMinutes:30,maxDiscoveryPages:3,autoImportFormat:'EPUB',requestIntervalMillis:1500,randomDelayMillis:1000,maxConcurrency:1,timeoutMillis:15000,retryCount:2,encoding:'UTF-8',userAgent:'',cookie:'',headersJson:'',proxies:[],contentFailureMarkers:[]})
...
const manualStatusOptions=[{value:'COMPLETED',label:'已完成（默认停止自动追更）'},{value:'PARTIAL_SUCCESS',label:'部分成功'},{value:'FAILED',label:'失败'},{value:'PAUSED',label:'已暂停'},{value:'WAITING',label:'等待中'},{value:'DISCOVERED',label:'已发现'}]
...
const sortedChapters=computed(()=>[...chapters.value].sort((left,right)=>{
  if(chapterSort.value==='indexAsc')return left.chapterIndex-right.chapterIndex
  if(chapterSort.value==='indexDesc')return right.chapterIndex-left.chapterIndex
  const timeDifference=(right.createdAt?Date.parse(right.createdAt):0)-(left.createdAt?Date.parse(left.createdAt):0)
...
onMounted(async()=>{await refresh();timer=window.setInterval(()=>{void pollCrawlerProgress()},3000)})
onUnmounted(()=>{if(timer)window.clearInterval(timer)})
async function refresh(){const [dashboardData,siteData,bookData,taskData]=await Promise.all([crawlerApi.dashboard(),crawlerApi.sites(),crawlerApi.books(),crawlerApi.tasks(),loadDiscoveredBooks()]);dashboard.value=dashboardData;sites.value=siteData;books.value=bookData;tasks.value=taskData}
async function pollCrawlerProgress(){
  if(progressPolling)return
  const hasActiveTask=tasks.value.some(task=>['RUNNING','WAITING'].includes(task.status))
...
async function syncOpenBookProgress(){
  ...
  const [latestBook,latestChapters,latestLogs]=await Promise.all([crawlerApi.book(bookId),crawlerApi.chapters(bookId),crawlerApi.logs(bookId)])
  if(!bookDrawer.value||selectedBook.value?.id!==bookId)return
  selectedBook.value=latestBook
  chapters.value=latestChapters
...
function siteTabForField(field:string):SiteEditorTab{
  ...
  if(['requestIntervalMillis','randomDelayMillis','maxConcurrency','timeoutMillis','retryCount','userAgent','cookie','headersJson'].includes(field))return'request'
  if(['enabled','autoScan','autoUpdate','autoCrawl','autoImportLibrary','scanIntervalMinutes','updateIntervalMinutes','maxDiscoveryPages','autoImportFormat'].includes(field))return'automation'
  return'basic'
}
...
async function openBook(book:CrawlerBook){selectedBook.value=book;chapters.value=[];crawlerLogs.value=[];bookDetailTab.value='chapters';bookDrawer.value=true;await syncOpenBookProgress()}
...
function openStatusEditor(book:CrawlerBook){statusBook.value=book;manualStatus.value=manualStatusOptions.some(item=>item.value===book.crawlStatus)?book.crawlStatus:'COMPLETED';manualAutoUpdateEnabled.value=book.crawlStatus==='COMPLETED'?false:Boolean(book.autoUpdateEnabled);statusDialog.value=true}
function handleManualStatusChange(status:string){if(status==='COMPLETED')manualAutoUpdateEnabled.value=false}
async function saveBookStatus(){if(!statusBook.value)return;savingStatus.value=true;try{const updated=await crawlerApi.setBookStatus(statusBook.value.id,manualStatus.value,manualAutoUpdateEnabled.value);books.value=books.value.map(book=>book.id===updated.id?updated:book);if(selectedBook.value?.id===updated.id)selectedBook.value=updated;statusDialog.value=false;message.success(`采集状态已改为“${statusLabel(updated.crawlStatus)}”，${updated.autoUpdateEnabled?'继续自动追更':'已停止自动追更'}`);await refresh()}finally{savingStatus.value=false}}
...
async function importBook(book:CrawlerBook){if(book.crawlStatus!=='COMPLETED'&&!await confirm(`“${book.bookName}”尚未完整采集，入库版本将只包含已有正文，是否继续？`))return;const result=await crawlerApi.importBook(book.id,'EPUB');message.success(`已有正文已加入书库，书籍 ID：${result.bookId}`);await refresh()}
...
async function removeTask(task:CrawlerTask){const updateNote=task.type==='BOOK_UPDATE_CHECK'?' 删除后该书将停止自动追更，不会再次自动创建更新任务。':'';if(!await confirm(`确定删除“${task.bookName||taskTypeLabel(task.type)}”的任务记录吗？关联书籍和章节不会被删除。${updateNote}`))return;await crawlerApi.deleteTask(task.id);message.success(task.type==='BOOK_UPDATE_CHECK'?'任务记录已删除，该书已停止自动追更':'任务记录已删除');await refresh()}
...
function taskTypeLabel(type:string){return({SITE_SCAN:'网站扫描',BOOK_METADATA:'书籍信息',BOOK_CHAPTER_LIST:'章节目录',BOOK_CONTENT:'章节正文',BOOK_UPDATE_CHECK:'更新检查',BOOK_FULL_CRAWL:'全本采集',BOOK_EXPORT:'文件生成',BOOK_IMPORT:'加入书库'} as Record<string,string>)[type]||type}
...
function handleChapterSortKey(e:KeyboardEvent){if(!['ArrowLeft','ArrowRight','Home','End'].includes(e.key))return;e.preventDefault();let index=chapterSortIndex.value;if(e.key==='ArrowRight')index=(index+1)%chapterSortOptions.length;else if(e.key==='ArrowLeft')index=(index-1+chapterSortOptions.length)%chapterSortOptions.length;else index=e.key==='Home'?0:chapterSortOptions.length-1;chapterSort.value=chapterSortOptions[index].value;requestAnimationFrame(()=>document.querySelectorAll<HTMLButtonElement>('.chapter-sort button')[index]?.focus())}
...
.discovery-pagination{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:18px 4px 2px;border-top:1px solid var(--border-color-light);color:var(--text-tertiary);font-size:12px}.discovery-pagination :deep(.el-pagination){min-width:0}@media(max-width:760px){.discovery-pagination{align-items:flex-start;flex-direction:column}.discovery-pagination :deep(.el-pagination){flex-wrap:wrap;justify-content:flex-start;gap:8px 0}}
.discovery-toolbar{display:grid;grid-template-columns:minmax(260px,1.6fr) minmax(160px,.8fr) minmax(190px,.9fr) auto auto;gap:10px;align-items:center;margin-bottom:16px;padding:14px;border:1px solid var(--border-color-light);border-radius:15px;background:var(--surface-elevated)}@media(max-width:980px){.discovery-toolbar{grid-template-columns:2fr 1fr 1fr}.discovery-toolbar .el-button{margin-left:0}}@media(max-width:640px){.discovery-toolbar{grid-template-columns:minmax(0,1fr)}.discovery-toolbar .el-button{width:100%}}
...
.site-form{max-height:none;min-height:0}.site-editor-tab-scroll{flex:0 0 auto;overflow-x:auto;padding-bottom:2px}.site-editor-tabs{position:relative;display:grid;width:max(100%,650px);grid-template-columns:repeat(5,minmax(130px,1fr));padding:4px;border:1px solid var(--border-color);border-radius:15px;background:var(--bg-page);isolation:isolate}.site-editor-tab-indicator{position:absolute;top:4px;bottom:4px;left:4px;z-index:0;width:calc((100% - 8px)/5);border:1px solid var(--border-color-light);border-radius:11px;background:var(--surface-elevated);box-shadow:var(--shadow-sm);transition:transform .28s cubic-bezier(.2,.8,.2,1)}.site-editor-tabs button{position:relative;z-index:1;display:grid;gap:2px;padding:9px 12px;border:0;background:transparent;color:var(--text-secondary);text-align:center;cursor:pointer}.site-editor-tabs button strong{font-size:13px}.site-editor-tabs button small{color:var(--text-tertiary);font-size:10px}.site-editor-tabs button.active,.site-editor-tabs button.active small{color:var(--primary)}.site-editor-tabs button:focus-visible{outline:2px solid var(--primary);outline-offset:-2px;border-radius:11px}.site-tab-panel{display:grid;gap:14px}:global(.site-editor-dialog){display:flex;max-height:90vh;flex-direction:column;margin-bottom:0}:global(.site-editor-dialog .el-dialog__body){display:flex;min-height:0;flex:1;overflow:hidden}:global(.site-editor-dialog .el-dialog__header),:global(.site-editor-dialog .el-dialog__footer){flex:0 0 auto}@media(prefers-reduced-motion:reduce){.site-editor-tab-indicator{transition:none}}
</style>
