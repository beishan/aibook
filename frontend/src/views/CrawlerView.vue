  ...
  <main class="crawler-page">
    ...
    <el-dialog v-model="statusDialog" title="人工修改采集状态" width="min(520px, 94vw)" append-to-body>
      <div v-if="statusBook" class="status-dialog-content">
        ...
        <el-alert type="warning" :closable="false" title="人工修改只调整整本书的采集状态，不会补齐缺失章节或改变章节统计。" />
      </div>
...
const manualStatusOptions=[{value:'COMPLETED',label:'已完成（采集成功）'},{value:'PARTIAL_SUCCESS',label:'部分成功'},{value:'FAILED',label:'失败'},{value:'PAUSED',label:'已暂停'},{value:'WAITING',label:'等待中'},{value:'DISCOVERED',label:'已发现'}]
...
async function saveBookStatus(){if(!statusBook.value)return;savingStatus.value=true;try{const updated=await crawlerApi.setBookStatus(statusBook.value.id,manualStatus.value);books.value=books.value.map(book=>book.id===updated.id?updated:book);if(selectedBook.value?.id===updated.id)selectedBook.value=updated;statusDialog.value=false;message.success(`采集状态已改为“${statusLabel(updated.crawlStatus)}”`);await refresh()}finally{savingStatus.value=false}}
...
async function removeTask(task:CrawlerTask){if(!await confirm(`确定删除“${task.bookName||taskTypeLabel(task.type)}”的任务记录吗？关联书籍和章节不会被删除。`))return;await crawlerApi.deleteTask(task.id);message.success('任务记录已删除');await refresh()}
