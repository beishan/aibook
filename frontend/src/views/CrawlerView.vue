<template>
  <main class="crawler-page">
    ...
    <el-dialog v-model="siteDialog" :title="editingSite ? '编辑采集网站' : '新增采集网站'" width="min(860px, 96vw)" destroy-on-close>
    ...
    <el-dialog v-model="ruleManagerDialog" :title="`${ruleSite?.siteName || ''} · 规则管理`" width="min(1120px, 96vw)" destroy-on-close>
      ...
      <el-table v-if="ruleVersions.length" :data="ruleVersions" class="rule-table" max-height="58vh" :row-class-name="({row}: {row:CrawlerRuleVersion}) => row.enabled ? 'active-rule-row' : ''">
        <el-table-column label="版本" width="92"><template #default="{row}"><strong class="rule-version-text">v{{ row.version }}</strong></template></el-table-column>
        <el-table-column label="状态" width="108"><template #default="{row}"><el-tag :type="row.enabled?'success':'info'" effect="light">{{ row.enabled?'当前生效':'已禁用' }}</el-tag></template></el-table-column>
        <el-table-column prop="changeSummary" label="版本说明" min-width="190" show-overflow-tooltip />
        <el-table-column label="书籍列表" min-width="150" show-overflow-tooltip><template #default="{row}"><code>{{ row.rule.discoveryItemSelector || '未配置' }}</code></template></el-table-column>
        <el-table-column label="章节目录" min-width="150" show-overflow-tooltip><template #default="{row}"><code>{{ row.rule.chapterItemSelector }}</code></template></el-table-column>
        <el-table-column label="正文" min-width="140" show-overflow-tooltip><template #default="{row}"><code>{{ row.rule.contentSelector }}</code></template></el-table-column>
        <el-table-column label="更新时间" width="168"><template #default="{row}">{{ formatTime(row.updatedAt || row.createdAt) }}</template></el-table-column>
        <el-table-column label="操作" width="360" fixed="right"><template #default="{row}"><div class="rule-actions"><el-button text :type="row.enabled?'warning':'success'" @click="toggleRule(row)">{{ row.enabled?'禁用':'启用' }}</el-button><el-button text :icon="Edit" @click="openRuleEditor(row)">编辑</el-button><el-button text :icon="Download" @click="exportRule(row)">导出</el-button><el-button text @click="ruleSite && openRuleTest(ruleSite,row.rule)">测试</el-button><el-button text type="danger" @click="removeRule(row)">删除</el-button></div></template></el-table-column>
      </el-table>
...
const siteDialog=ref(false), crawlDialog=ref(false), bookDrawer=ref(false), chapterDialog=ref(false), ruleTestDialog=ref(false), ruleManagerDialog=ref(false), ruleEditorDialog=ref(false), ruleImportDialog=ref(false), saving=ref(false), testingRule=ref(false), editingSite=ref<CrawlerSite>(), selectedBook=ref<CrawlerBook>(), selectedDiscoveries=ref<CrawlerBook[]>([]), chapters=ref<CrawlerChapter[]>([]), chapterDetail=ref<{title:string;url:string;content:string;errorMessage:string}>(), bookKeyword=ref('')
...
const emptySite=():CrawlerSitePayload=>({siteName:'',siteCode:'',baseUrl:'',homeUrl:'',enabled:false,autoScan:false,autoCrawl:false,autoUpdate:true,autoImportLibrary:false,scanIntervalMinutes:360,updateIntervalMinutes:30,maxDiscoveryPages:3,autoImportFormat:'EPUB',requestIntervalMillis:1500,randomDelayMillis:1000,maxConcurrency:1,timeoutMillis:15000,retryCount:2,encoding:'UTF-8',userAgent:'',cookie:'',headersJson:'',proxies:[]})
...
async function saveSite(){const valid=await siteFormRef.value?.validate().catch(()=>false);if(!valid)return; saving.value=true;try{editingSite.value?await crawlerApi.updateSite(editingSite.value.id,siteForm):await crawlerApi.createSite(siteForm);message.success(editingSite.value?'网站信息已保存':'网站已创建，请在卡片的规则管理中添加规则');siteDialog.value=false;await refresh()}finally{saving.value=false}}
