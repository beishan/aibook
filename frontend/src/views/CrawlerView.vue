...
async function importBook(book:CrawlerBook){if(book.crawlStatus!=='COMPLETED'&&!await confirm(`“${book.bookName}”尚未完整采集，入库版本将只包含已有正文，是否继续？`))return;const syncing=book.importStatus==='IMPORTED',result=await crawlerApi.importBook(book.id,'EPUB');message.success(syncing?`书库同步完成；内容未变化时不会重复创建版本（书籍 ID：${result.bookId}）`:`已有正文已加入书库，书籍 ID：${result.bookId}`);await refresh()}
