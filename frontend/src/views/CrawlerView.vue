...
async function syncOpenBookProgress(){
  ...
  try{
    const [latestBook,latestChapters,latestLogs]=await Promise.all([crawlerApi.book(bookId),crawlerApi.chapters(bookId,{page:chapterPage.value-1,size:chapterPageSize.value,sort:chapterSortApiValue(chapterSort.value)}),crawlerApi.logs(bookId)])
...
async function changeChapterSort(value:ChapterSort){if(chapterSort.value===value)return;chapterSort.value=value;chapterPage.value=1;await syncOpenBookProgress()}
...
async function openBook(book:CrawlerBook){selectedBook.value=book;chapters.value=[];chapterTotal.value=book.chapterCount;chapterPage.value=1;crawlerLogs.value=[];bookDetailTab.value='chapters';bookDrawer.value=true;await syncOpenBookProgress()}
