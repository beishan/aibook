<template>
  <main class="crawler-page">
    ...
    <el-dialog v-model="queuedTasksDialog" :title="`当前排队任务 · ${queuedTasks.length}`" width="min(760px, 94vw)" append-to-body>
      <div v-if="queuedTasks.length" class="queued-order-note"><span class="queue-drag-mark">⠿</span><div><strong>拖动调整执行顺序</strong><small>同一优先级内越靠上越先执行；跨优先级请先在任务详情中修改优先级。</small></div></div>
      <el-table v-loading="queuedTasksLoading||queuedTasksReordering" :data="queuedTasks" row-key="id" max-height="56vh" class="queued-task-table" :row-class-name="queuedTaskRowClassName">
        <el-table-column label="排序" width="62" align="center"><template #default="{row}"><button type="button" class="queue-drag-handle" :class="{dragging:queuedTaskDraggingId===row.id}" :draggable="!queuedTasksReordering" :aria-label="`拖动排序：${row.bookName||row.discoveryPageName||taskTypeLabel(row.type)}`" title="拖动调整顺序；也可使用上下方向键" @dragstart="startQueuedTaskDrag(row,$event)" @dragenter.prevent="moveDraggedQueuedTask(row)" @dragover.prevent @dragend="finishQueuedTaskDrag" @keydown.up.prevent="moveQueuedTaskByKeyboard(row,-1)" @keydown.down.prevent="moveQueuedTaskByKeyboard(row,1)">⠿</button></template></el-table-column>
...
const queueSettingsDialog=ref(false), savingQueueSettings=ref(false), taskQueueSettings=ref<CrawlerTaskQueueSettings>(), queueLimit=ref(4), queuedTasksDialog=ref(false), queuedTasksLoading=ref(false), queuedTasksReordering=ref(false), queuedTaskDraggingId=ref<string>(), queuedTaskDragStartOrder=ref<string[]>([]), queuedTasks=ref<CrawlerTask[]>([])
...
async function loadQueuedTasks(options:LoadOptions={}){if(options.silent&&(queuedTaskDraggingId.value||queuedTasksReordering.value))return;if(!options.silent)queuedTasksLoading.value=true;try{queuedTasks.value=await crawlerApi.queuedTasks()}finally{if(!options.silent)queuedTasksLoading.value=false}}
