  ...
  <main class="crawler-page">
    ...
    <section v-else-if="activeTab === 'discovered'" class="panel" role="tabpanel">
      ...
      <el-table v-if="discoveryViewMode==='table'" v-loading="discoveryLoading" :data="discoveredBooks" class="data-table" @selection-change="selectedDiscoveries=$event">
        ...
        <el-table-column label="操作" width="340" fixed="right" align="right"><template #default="{row}"><div class="crawler-book-action-cluster discovery-book-action-cluster" @click.stop><el-button size="small" round class="crawler-book-action-button crawler-book-action-details" @click="openBook(row)">详情</el-button><el-button size="small" round class="favorite-action" :class="{'is-favorite':row.favorite}" @click="toggleFavorite(row)">{{ row.favorite?'已收藏':'收藏' }}</el-button><el-button size="small" round class="crawler-book-action-button crawler-book-action-import" @click="crawlDiscovered(row)">采集</el-button><el-dropdown trigger="click" placement="bottom-end" popper-class="discovery-more-popper" @command="handleDiscoveryCardMore($event,row)"><el-button size="small" circle :icon="MoreFilled" class="crawler-book-action-more" aria-label="更多操作" title="更多操作" /><template #dropdown><el-dropdown-menu><el-dropdown-item command="book-lists">加入书单</el-dropdown-item><el-dropdown-item command="metadata" :disabled="isBookTaskActive(row)">刷新分类标签</el-dropdown-item><el-dropdown-item command="website">查看网站</el-dropdown-item><el-dropdown-item command="ignore" divided>忽略</el-dropdown-item><el-dropdown-item command="blacklist" class="danger-dropdown-item">加入黑名单</el-dropdown-item></el-dropdown-menu></template></el-dropdown></div></template></el-table-column>
      </el-table>
        ...
        <div class="discovery-card-grid">
          <article v-for="book in discoveredBooks" :key="book.id" class="discovery-card" :class="{selected:isDiscoverySelected(book)}">
            <div class="discovery-card-action-overlay" role="group" :aria-label="`${book.bookName}快捷操作`" @click.stop>
              ...
              <el-button size="small" class="favorite-action" :class="{'is-favorite':book.favorite}" @click="toggleFavorite(book)">{{ book.favorite?'已收藏':'收藏' }}</el-button>
    ...
    <section v-else-if="activeTab === 'books'" class="panel" role="tabpanel">
      ...
      <el-table v-if="bookViewMode==='table'" ref="bookTableRef" v-loading="bookLoading" :data="books" row-key="id" @selection-change="selectedBooks=$event" @row-click="handleBookTableRowClick" class="data-table">
        ...
        <el-table-column label="操作" width="340" fixed="right" align="right"><template #default="{row}"><div class="crawler-book-action-cluster" @click.stop><el-button size="small" round class="favorite-action" :class="{'is-favorite':row.favorite}" @click="toggleFavorite(row)">{{ row.favorite?'已收藏':'收藏' }}</el-button><el-button size="small" round class="crawler-book-action-button crawler-book-action-details" @click="openBook(row)">详情</el-button><el-button size="small" round class="crawler-book-action-button crawler-book-action-continue" :disabled="isBookTaskActive(row)" @click="continueCrawl(row)">{{ isBookTaskActive(row)?'任务中':'继续' }}</el-button><el-button size="small" round class="crawler-book-action-button crawler-book-action-import" @click="importBook(row)">{{ row.importStatus==='IMPORTED'?'同步入库':'入库' }}</el-button><el-dropdown trigger="click" placement="bottom-end" popper-class="discovery-more-popper" @command="handleCrawlerBookTableMore($event,row)"><el-button size="small" circle :icon="MoreFilled" class="crawler-book-action-more" aria-label="更多操作" title="更多操作" /><template #dropdown><el-dropdown-menu><el-dropdown-item command="book-lists">加入书单</el-dropdown-item><el-dropdown-item command="updates">检查更新</el-dropdown-item><el-dropdown-item command="metadata" :disabled="isBookTaskActive(row)">刷新分类标签</el-dropdown-item><el-dropdown-item command="trial" :disabled="!row.crawledChapterCount">试读</el-dropdown-item><el-dropdown-item command="generate">生成文件</el-dropdown-item></el-dropdown-menu></template></el-dropdown></div></template></el-table-column>
      </el-table>
    ...
    <el-dialog v-model="queuedTasksDialog" :title="`当前任务 · ${queuedTasks.length}`" width="min(1040px, calc(100vw - 32px))" class="queued-tasks-dialog" append-to-body>
      ...
      <el-table v-loading="queuedTasksLoading||queuedTasksReordering||!!queuedTaskPrioritizingId" :data="pagedQueuedTasks" row-key="id" max-height="56vh" class="queued-task-table" :row-class-name="queuedTaskRowClassName">
        ...
        <el-table-column label="操作" width="250" fixed="right" align="right"><template #default="{row}"><div class="queued-task-actions"><el-button size="small" round class="queued-task-action-details" :disabled="Boolean(queuedTaskCommandId)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="openQueuedTask(row)">详情</el-button><el-button v-if="['RUNNING','WAITING'].includes(row.status)" size="small" round class="queued-task-action-pause" :loading="queuedTaskCommandId===row.id&&queuedTaskCommand==='pause'" :disabled="Boolean(queuedTaskCommandId&&queuedTaskCommandId!==row.id)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="commandCurrentTask(row,'pause')">暂停</el-button><el-button v-if="row.status==='PAUSED'" size="small" round class="queued-task-action-resume" :loading="queuedTaskCommandId===row.id&&queuedTaskCommand==='resume'" :disabled="Boolean(queuedTaskCommandId&&queuedTaskCommandId!==row.id)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="commandCurrentTask(row,'resume')">继续</el-button><el-button v-if="row.status==='WAITING'" size="small" round class="queued-task-action-prioritize" :loading="queuedTaskPrioritizingId===row.id" :disabled="queuedTasksReordering||Boolean(queuedTaskCommandId)||Boolean(queuedTaskPrioritizingId&&queuedTaskPrioritizingId!==row.id)" @click="prioritizeQueuedTask(row)">优先</el-button><el-button v-if="['RUNNING','WAITING','PAUSED'].includes(row.status)" size="small" round class="queued-task-action-cancel" :loading="queuedTaskCommandId===row.id&&queuedTaskCommand==='cancel'" :disabled="Boolean(queuedTaskCommandId&&queuedTaskCommandId!==row.id)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="commandCurrentTask(row,'cancel')">取消</el-button></div></template></el-table-column>
      </el-table>
...
const TaskTable = defineComponent({ props:{ tasks:{type:Array as ()=>CrawlerTask[],required:true},selectable:{type:Boolean,default:false}}, emits:['open','command','edit','delete','scan-results','selection-change','toggle-favorite','book-lists'], setup(props,{emit}) {
  ...
  h(ElTableColumn,{label:'操作',width:340,fixed:'right',align:'right'},{default:({row}:{row:CrawlerTask})=>{
    ...
    return h('div',{class:'task-action-cluster'},[
      ...
      row.bookId?h(ElButton,{size:'small',round:true,class:['favorite-action',row.favorite?'is-favorite':''],onClick:()=>emit('toggle-favorite',row)},()=>row.favorite?'已收藏':'收藏'):null,
...
:deep(.favorite-action.is-favorite){border-color:#d59b26!important;background:#f6c85f!important;color:#3b2500!important;font-weight:800}:deep(.favorite-action.is-favorite:hover),:deep(.favorite-action.is-favorite:focus-visible){border-color:#c88b12!important;background:#ffd978!important;color:#2d1b00!important}
