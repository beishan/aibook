<template>
  <main class="crawler-page">
          ...
          <article v-for="book in discoveredBooks" :key="book.id" class="discovery-card" :class="{selected:isDiscoverySelected(book)}">
            <div class="discovery-card-action-overlay" role="group" :aria-label="`${book.bookName}快捷操作`" @click.stop>
              ...
              <el-dropdown trigger="click" placement="bottom-end" popper-class="discovery-more-popper" @command="handleDiscoveryCardMore($event,book)">
                ...
                <template #dropdown><el-dropdown-menu><el-dropdown-item command="details">详细信息</el-dropdown-item><el-dropdown-item command="website">查看网站</el-dropdown-item><el-dropdown-item command="blacklist" divided class="danger-dropdown-item">加入黑名单</el-dropdown-item></el-dropdown-menu></template>
              </el-dropdown>
            </div>
            ...
            <div v-if="book.category" class="discovery-card-body discovery-card-category"><el-tag size="small" effect="plain">{{ book.category }}</el-tag></div>
          </article>
        </div>
      </div>
    ...
    <section v-else-if="activeTab === 'books'" class="panel" role="tabpanel">
      ...
      <transition name="task-batch-rise">
        <div v-if="selectedBooks.length" class="task-batch-bar book-batch-bar" role="toolbar" aria-label="采集书籍批量修改">
          ...
          <div class="book-batch-actions"><div class="book-batch-task-actions"><el-button :loading="batchBookAction==='continue'" :disabled="batchBookBusy||!selectedBooks.some(book=>!isBookTaskActive(book))" @click="batchContinueBooks">继续</el-button><el-button :loading="batchBookAction==='updates'" :disabled="batchBookBusy||!selectedBooks.some(book=>!isBookTaskActive(book))" @click="batchCheckUpdates">检查更新</el-button><el-button type="primary" plain :disabled="batchBookBusy" @click="openBatchImport">入库</el-button></div><div class="book-batch-status-actions"><el-select v-model="batchBookStatus" aria-label="批量设置采集状态"><el-option v-for="item in manualStatusOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select><el-button type="primary" :loading="batchBookStatusSaving" :disabled="batchBookBusy" @click="saveBatchBookStatus">统一更改状态</el-button></div></div>
        </div>
      </transition>
      <el-table v-if="bookViewMode==='table'" ref="bookTableRef" v-loading="bookLoading" :data="books" row-key="id" @selection-change="selectedBooks=$event" @row-click="handleBookTableRowClick" class="data-table">
        ...
        <el-table-column label="书籍" min-width="260"><template #default="{row}"><div class="book-cell"><div class="mini-cover">{{ row.bookName.slice(0,1) }}</div><div><strong>{{ row.bookName }}</strong><p>{{ row.author || '未知作者' }} · {{ row.siteName }}</p></div></div></template></el-table-column>
        <el-table-column label="进度" min-width="210"><template #default="{row}"><el-progress :class="{'crawler-running-progress':isBookRunning(row)}" :percentage="progress(row)" :stroke-width="7" /><small>正文 {{ row.crawledChapterCount }}，待开放 {{ row.pendingReleaseChapterCount }} / 共 {{ row.chapterCount }} 章</small></template></el-table-column>
        ...
        <el-table-column label="操作" width="350"><template #default="{row}"><el-button text @click.stop="checkUpdates(row)">检查更新</el-button><el-button text :disabled="isBookTaskActive(row)" @click.stop="continueCrawl(row)">{{ isBookTaskActive(row)?'任务中':'继续' }}</el-button><el-button text type="primary" :disabled="!row.crawledChapterCount" @click.stop="startTrial(row)">试读</el-button><el-button text @click.stop="generate(row)">生成</el-button><el-button text type="primary" @click.stop="importBook(row)">{{ row.importStatus==='IMPORTED'?'同步入库':'入库' }}</el-button></template></el-table-column>
      </el-table>
          ...
          <article v-for="book in books" :key="book.id" class="discovery-card crawler-book-card" :class="{selected:isBookSelected(book)}" @click="openBook(book)">
            <div class="discovery-card-action-overlay crawler-book-action-overlay" role="group" :aria-label="`${book.bookName}快捷操作`" @click.stop>
              ...
              <el-dropdown trigger="click" placement="bottom-end" popper-class="discovery-more-popper" @command="handleCrawlerBookCardMore($event,book)">
                <el-button size="small" aria-label="更多操作">更多</el-button>
                <template #dropdown><el-dropdown-menu><el-dropdown-item command="details">详细信息</el-dropdown-item><el-dropdown-item command="website">查看网站</el-dropdown-item><el-dropdown-item command="updates" divided>检查更新</el-dropdown-item><el-dropdown-item command="generate">生成文件</el-dropdown-item><el-dropdown-item command="import">{{ book.importStatus==='IMPORTED'?'同步入库':'加入书库' }}</el-dropdown-item><el-dropdown-item command="status">修改状态</el-dropdown-item></el-dropdown-menu></template>
              </el-dropdown>
            </div>
      ...
      <transition name="task-batch-rise">
        <div v-if="selectedTaskRows.length" class="task-batch-bar" role="toolbar" aria-label="采集任务批量管理">
          ...
          <div class="task-batch-actions">
            ...
            <el-button :disabled="batchTaskManaging || !canBatchPause" :loading="batchTaskManaging && batchTaskAction==='pause'" @click="manageSelectedTasks('pause')">暂停</el-button>
            <el-button type="primary" plain :disabled="batchTaskManaging || !canBatchResume" :loading="batchTaskManaging && batchTaskAction==='resume'" @click="manageSelectedTasks('resume')">继续</el-button>
    ...
    <el-dialog v-model="queuedTasksDialog" :title="`当前任务 · ${queuedTasks.length}`" width="min(1040px, calc(100vw - 32px))" class="queued-tasks-dialog" append-to-body>
      <div v-if="queuedTasks.some(task=>task.status==='WAITING')" class="queued-order-note"><span class="queue-drag-mark">⠿</span><div><strong>拖动调整等待顺序</strong><small>只有等待中的任务可以排序；需要置顶时可点击“优先”。</small></div></div>
      <el-table v-loading="queuedTasksLoading||queuedTasksReordering||!!queuedTaskPrioritizingId" :data="queuedTasks" row-key="id" max-height="56vh" class="queued-task-table" :row-class-name="queuedTaskRowClassName">
        ...
        <el-table-column label="操作" width="230" fixed="right"><template #default="{row}"><div class="queued-task-actions"><el-button v-if="['RUNNING','WAITING'].includes(row.status)" text :loading="queuedTaskCommandId===row.id&&queuedTaskCommand==='pause'" :disabled="Boolean(queuedTaskCommandId&&queuedTaskCommandId!==row.id)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="commandCurrentTask(row,'pause')">暂停</el-button><el-button v-if="row.status==='PAUSED'" text type="primary" :loading="queuedTaskCommandId===row.id&&queuedTaskCommand==='resume'" :disabled="Boolean(queuedTaskCommandId&&queuedTaskCommandId!==row.id)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="commandCurrentTask(row,'resume')">继续</el-button><el-button v-if="row.status==='WAITING'" text type="primary" :loading="queuedTaskPrioritizingId===row.id" :disabled="queuedTasksReordering||Boolean(queuedTaskCommandId)||Boolean(queuedTaskPrioritizingId&&queuedTaskPrioritizingId!==row.id)" @click="prioritizeQueuedTask(row)">优先</el-button><el-button text :disabled="Boolean(queuedTaskCommandId)||queuedTasksReordering||Boolean(queuedTaskPrioritizingId)" @click="openQueuedTask(row)">详情</el-button></div></template></el-table-column>
      </el-table>
    ...
    <el-drawer v-model="bookDrawer" size="min(760px, 96vw)" :title="selectedBook?.bookName || '采集书籍详情'" class="book-detail-drawer">
      <div v-if="selectedBook" class="book-drawer-content">
        <div class="book-summary"><div class="large-cover">{{ selectedBook.bookName.slice(0,1) }}</div><div><h2>{{ selectedBook.bookName }}</h2><p>{{ selectedBook.author || '未知作者' }} · {{ selectedBook.siteName }}</p><el-progress :class="{'crawler-running-progress':isBookRunning(selectedBook)}" :percentage="progress(selectedBook)"/><small>正文 {{ selectedBook.crawledChapterCount }} / {{ selectedBook.chapterCount }} 章，待开放 {{ selectedBook.pendingReleaseChapterCount }}，失败 {{ selectedBook.failedChapterCount }}</small></div></div>
...
import { Collection, Connection, DataAnalysis, Document, Download, Edit, Link, List, MoreFilled, Plus, Refresh, Search, Tickets, Upload, Warning } from '@element-plus/icons-vue'
import { ElButton, ElButtonGroup, ElDropdown, ElDropdownItem, ElDropdownMenu, ElProgress, ElTable, ElTableColumn, ElTag, type FormInstance, type FormItemRule, type FormRules } from 'element-plus'
...
const TaskTable = defineComponent({ props:{ tasks:{type:Array as ()=>CrawlerTask[],required:true},selectable:{type:Boolean,default:false}}, emits:['open','command','edit','delete','scan-results','selection-change'], setup(props,{emit}) {
  ...
  h(ElTableColumn,{label:'操作',width:250,fixed:'right'},{default:({row}:{row:CrawlerTask})=>{
    const actions=moreActions(row)
    return h(ElButtonGroup,{class:'task-action-button-group'},()=>[
      h(ElButton,{size:'small',type:'primary',plain:true,onClick:()=>emit('open',row)},()=> '详情'),
      ['RUNNING','WAITING'].includes(row.status)?h(ElButton,{size:'small',onClick:()=>emit('command',row,'pause')},()=> '暂停'):null,
      ['RUNNING','WAITING','PAUSED'].includes(row.status)?h(ElButton,{size:'small',type:'danger',plain:true,onClick:()=>emit('command',row,'cancel')},()=> '取消'):null,
      actions.length?h(ElDropdown,{trigger:'click',placement:'bottom-end',onCommand:(command:TaskMoreCommand)=>handleMoreCommand(row,command)},{
        default:()=>h(ElButton,{size:'small',icon:MoreFilled,'aria-label':'更多操作',title:'更多操作'}),
...
const queueSettingsDialog=ref(false), savingQueueSettings=ref(false), taskQueueSettings=ref<CrawlerTaskQueueSettings>(), queueLimit=ref(4), queuedTasksDialog=ref(false), queuedTasksLoading=ref(false), queuedTasksReordering=ref(false), queuedTaskPrioritizingId=ref<string>(), queuedTaskCommandId=ref<string>(), queuedTaskCommand=ref<'pause'|'resume'>(), queuedTaskDraggingId=ref<string>(), queuedTaskDragStartOrder=ref<string[]>([]), queuedTasks=ref<CrawlerTask[]>([])
...
const emptySite=():CrawlerSitePayload=>({siteName:'',siteCode:'',baseUrl:'',homeUrl:'',enabled:false,autoScan:false,autoCrawl:false,autoUpdate:false,autoImportLibrary:false,scanIntervalMinutes:360,updateIntervalMinutes:30,maxDiscoveryPages:3,autoImportFormat:'EPUB',requestIntervalMillis:1500,randomDelayMillis:1000,maxConcurrency:1,encoding:'UTF-8',proxies:[],contentMarkers:[]})
const siteConfigurationTemplate:CrawlerSiteConfiguration={
  schemaVersion:1,
  type:'AIBOOK_CRAWLER_SITE',
  site:{siteName:'示例小说站',siteCode:'example_novel',baseUrl:'https://www.example.com',homeUrl:'https://www.example.com',enabled:false,autoScan:false,autoCrawl:false,autoUpdate:false,autoImportLibrary:false,requestIntervalMillis:1500,randomDelayMillis:1000,maxConcurrency:1,encoding:'UTF-8',proxies:[{name:'备用代理',url:'http://127.0.0.1:7890',enabled:false}],scanIntervalMinutes:360,updateIntervalMinutes:30,maxDiscoveryPages:3,autoImportFormat:'EPUB',contentMarkers:[{marker:'以下内容为VIP专属，升级会员即可继续阅读',status:'PENDING_RELEASE'},{marker:'Access Denied',status:'FAILED'}]},
...
async function commandCurrentTask(task:CrawlerTask,command:'pause'|'resume'){if(queuedTaskCommandId.value||queuedTasksReordering.value||queuedTaskPrioritizingId.value)return;queuedTaskCommandId.value=task.id;queuedTaskCommand.value=command;try{await crawlerApi.taskCommand(task.id,command);message.success(`“${task.bookName||task.discoveryPageName||taskTypeLabel(task.type)}”已${command==='pause'?'暂停':'继续'}`);const [dashboardData]=await Promise.all([crawlerApi.dashboard(),loadQueuedTasks(),loadTasks({silent:true}),loadFailedTasks({silent:true}),loadTaskQueueSettings()]);dashboard.value=dashboardData}catch(error:any){message.error(error.response?.data?.message||`任务${command==='pause'?'暂停':'继续'}失败，请稍后重试`);await loadQueuedTasks()}finally{queuedTaskCommandId.value=undefined;queuedTaskCommand.value=undefined}}
...
function handleDiscoveryCardMore(command:string,book:CrawlerBook){if(command==='details')void openBook(book);else if(command==='website')window.open(book.bookUrl,'_blank','noopener,noreferrer');else if(command==='blacklist')void batchDiscovery('BLACKLISTED',[book.id])}
function handleCrawlerBookCardMore(command:string,book:CrawlerBook){if(command==='details')void openBook(book);else if(command==='website')window.open(book.bookUrl,'_blank','noopener,noreferrer');else if(command==='updates')void checkUpdates(book);else if(command==='generate')void generate(book);else if(command==='import')importBook(book);else if(command==='status')openStatusEditor(book)}
...
.task-action-button-group{display:inline-flex;white-space:nowrap}.task-action-button-group :deep(.el-dropdown){display:inline-flex}.task-action-button-group :deep(.el-dropdown .el-button){margin-left:-1px;border-top-left-radius:0;border-bottom-left-radius:0}.task-action-button-group:has(.el-dropdown) :deep(>.el-button:last-of-type){border-top-right-radius:0;border-bottom-right-radius:0}
