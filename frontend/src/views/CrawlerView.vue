<template>
  <main class="crawler-page">
    ...
    <section v-if="activeTab === 'overview'" class="panel" role="tabpanel">
      ...
      <section v-loading="statisticsLoading" class="crawler-statistics" aria-labelledby="crawler-statistics-title">
...
const chapterTrendChartRef=ref<HTMLElement>(), bookTrendChartRef=ref<HTMLElement>(), successRateChartRef=ref<HTMLElement>(), siteContributionChartRef=ref<HTMLElement>(), funnelChartRef=ref<HTMLElement>()
...
const statisticsTaskTotals=computed(()=>statistics.value?.daily.reduce((totals,item)=>({finished:totals.finished+item.finishedTasks,successful:totals.successful+item.successfulTasks}),{finished:0,successful:0})||{finished:0,successful:0})
...
watch(activeTab,async value=>{if(value==='overview'&&statistics.value){await nextTick();await renderStatisticsCharts()}})
...
async function loadCrawlerStatistics(options:{silent?:boolean}={}){if(!options.silent)statisticsLoading.value=true;try{statistics.value=await crawlerApi.dashboardStatistics(statisticsDays.value);if(activeTab.value==='overview'){await nextTick();await renderStatisticsCharts()}}catch(error:any){if(!options.silent)message.error(error.response?.data?.message||'采集统计加载失败')}finally{if(!options.silent)statisticsLoading.value=false}}
...
:deep(.el-table__row .row-hover-action){opacity:0;pointer-events:none;transform:translateY(2px);transition:opacity .16s ease,transform .16s ease}:deep(.el-table__row:hover .row-hover-action),:deep(.el-table__row:focus-within .row-hover-action){opacity:1;pointer-events:auto;transform:none}@media(hover:none){:deep(.el-table__row .row-hover-action){opacity:1;pointer-events:auto;transform:none}}@media(prefers-reduced-motion:reduce){:deep(.el-table__row .row-hover-action){transition:none}}
