...
### 配置持久化规范
- 后续新增或调整用户配置、界面偏好和功能开关时，默认绑定当前账户并由服务端持久化，使同一账户可跨浏览器、设备和会话恢复。
- 浏览器本地存储仅用于未登录状态、账户偏好的本地缓存或旧版本数据迁移，不得作为已登录用户配置的唯一存储位置。
- 配置数据体积较大、不适合直接保存到账户时，必须先向用户说明数据规模、服务端与浏览器存储的取舍并获得确认后再实现。
