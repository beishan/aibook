...
Jenkins 会从 `frontend/package.json` 读取应用版本，前后端镜像标签统一为
`应用版本-构建号-提交短哈希`，例如 `aibook-backend:1.0.2-128-a1b2c3d4e5f6` 和
`aibook-frontend:1.0.2-128-a1b2c3d4e5f6`。应用版本便于识别发布内容，构建号和提交哈希
保证同一应用版本重复构建时仍使用唯一标签。
